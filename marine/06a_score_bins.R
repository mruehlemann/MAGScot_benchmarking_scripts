...
bisco_ms80 = read.table(paste0(sample,".MAGScoT_ms80.refined.contig_to_bin.out"), head=T, stringsAsFactors=F) %>% mutate(set="MAGScoT_ms80")
bisco_ms50 = read.table(paste0(sample,".MAGScoT.refined.contig_to_bin.out"), head=T, stringsAsFactors=F) %>% mutate(set="MAGScoT_ms50")
...
goldst = read.table(paste0(sample,".gs.contig_to_bin.tsv"), head=F, stringsAsFactors=F, sep="\t")
colnames(goldst) = c("bin","contig","set")

allbins = bind_rows(binners_outputs, allbisco, metawrap, das_tool, goldst) %>% mutate(bin=gsub("[.]fasta$","",bin))
